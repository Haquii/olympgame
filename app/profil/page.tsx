      ...
      <div className="container-app pt-12 max-w-[480px]">
        <div className="card p-7">
          ...
          <p className="text-ink-soft mb-5">
            Crée ton profil pour rejoindre ou organiser des tournois.
          </p>
  ...
  return (
    <div className="container-app pt-8">
      <div className="card p-7 bg-grad-hero text-white border-0 mb-6 grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-5 items-center">
        ...
        <div>
          <h1 className="font-display text-[40px] tracking-wider leading-none">
            {me.name}
          </h1>
          <p className="text-sm opacity-85 mt-1">
            Membre depuis {fmtDate(me.joinedAt)}
          </p>
        </div>
        <div className="flex flex-col gap-2 md:items-end">
          <button
            onClick={() => setEditOpen(true)}
            className="btn text-white bg-white/15 border border-white/30 hover:bg-white/25 btn-sm"
          >
            ✏️ Modifier
          </button>
          <button
            ...
            className="btn text-white/80 bg-white/5 border border-white/20 hover:bg-white/10 btn-sm"
          >
            🔄 Reset démo
          </button>
        </div>
      ...
      <section className="mb-10">
        <h2 className="font-display text-[30px] tracking-wider leading-none mb-4">
          Mes tournois ({myTournaments.length})
        </h2>
        {myTournaments.length === 0 ? (
          <Empty icon="🎮" title="Aucun tournoi rejoint">
            <button
      ...
      <section>
        <h2 className="font-display text-[30px] tracking-wider leading-none mb-4">
          Tournois que j&apos;organise ({myOrganized.length})
        </h2>
        {myOrganized.length === 0 ? (
          <Empty icon="⚙️" title="Tu n'organises encore rien">
            <button
...
function Stat({
  ...
  return (
    <div className="card text-center">
      <div className={`font-display text-[36px] leading-none ${color}`}>
        {value}
      </div>
      <div className="text-xs text-ink-mute uppercase tracking-wider mt-1.5">
        {label}
      </div>
    </div>
  );
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <input
            ...
            className="h-11 w-full p-1 border border-border rounded-lg"
          />
        </div>
      </div>
    </Modal>
