...
export default function TabOrga({ t }: { t: Tournament }) {
  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="card">
          ...
          <p className="text-ink-soft text-sm mb-4">
            Les co-organisateurs peuvent ajouter des jeux, saisir les résultats et
            gérer les joueurs.
          </p>
          <div className="flex flex-col gap-2">
            {orgs.map((u) => {
              ...
              return (
                <div
                  key={u.id}
                  className="flex justify-between items-center p-2.5 border border-border rounded-lg"
                >
                  ...
                  {isCreator && u.id !== t.createdBy && (
                    <button
                      onClick={() => {
                        removeOrganizer(t.id, u.id);
                        toast("Co-organisateur retiré");
                      }}
          ...
          <div className="flex flex-col gap-3.5">
            ...
            <div>
              ...
              <input
                ...
                value={t.startDate}
                onChange={(e) => {
                  updateTournament(t.id, { startDate: e.target.value });
                }}
              />
            </div>
            <div>
              ...
              <input
                ...
                value={t.maxPlayers}
                onChange={(e) => {
                  const v = parseInt(e.target.value) || t.maxPlayers;
                  updateTournament(t.id, { maxPlayers: v });
                }}
              />
            </div>
            <div>
              ...
              <input
                ...
                value={t.name}
                onChange={(e) => {
                  updateTournament(t.id, { name: e.target.value });
                }}
              />
            </div>
            <div>
              ...
              <textarea
                ...
                value={t.description}
                onChange={(e) => {
                  updateTournament(t.id, { description: e.target.value });
                }}
              />
            </div>
            {isCreator && (
              <button onClick={onDelete} className="btn btn-danger btn-block mt-2">
      ...
      {addOrgaOpen && (
        <AddOrgaModal tid={t.id} onClose={() => setAddOrgaOpen(false)} />
      )}
    </>
  );
}

function AddOrgaModal({
  tid,
  onClose,
}: {
  tid: string;
    ...
    return (
      ...
      >
        <p className="text-ink-soft">
          Tous les utilisateurs sont déjà organisateurs.
        </p>
      </Modal>
    );
  }
  ...
  return (
      ...
      footer={
        <>
          ...
          <button
            onClick={() => {
              if (!pid) return;
              addOrganizer(tid, pid);
              toast("Co-organisateur ajouté", "success");
              onClose();
            }}
    ...
    >
      <p className="text-ink-soft text-sm mb-4">
        Il pourra ajouter des jeux, saisir les résultats et gérer les joueurs.
      </p>
