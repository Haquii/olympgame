...
export default function PointsEditor({
  ...
  const addRow = () => {
    update([...rows, { rank: rows.length + 1, points: 0 }]);
  };

  const removeRow = (i: number) => {
    const next = rows.filter((_, idx) => idx !== i).map((r, idx) => ({
      rank: idx + 1,
      points: r.points,
    }));
    update(next);
  ...
  const setPoints = (i: number, v: string) => {
    ...
    const next = rows.map((r, idx) => (idx === i ? { ...r, points: n } : r));
    update(next);
  };
      ...
      {rows.map((p, i) => (
        <div key={i} className="flex items-center gap-2.5">
          ...
          <button
            ...
            aria-label="Supprimer cette place"
          >
            ×
          </button>
        </div>
